...
Therefore, we modified our approach to sequence model only the classes at the same level, rather than all 25 classes. This adjustment significantly improved our score. 
...
**stage1**
- max length
- cnn backbone(regnety_002, efficientnet_b5)
- whether it has fixed (x, y) coordinates or dynamic (x, y) coordinates according to the z-axis.

**stage2**
- rnn modeling(skip connection, sequence modeling axis)
- cnn backbone(convnext_small, convnext_tiny, caformer_s18, pvt_v2_b3)
